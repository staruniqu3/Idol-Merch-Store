    ...
    try {
      ...
      for (const item of cart) {
        const res = await fetch(`${base}/api/slot-bookings`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ productId: item.productId, variant: item.variant ?? null, subVariant: item.subVariant ?? null, phone: phone.trim(), socialHandle: socialHandle.trim() || null, quantity: item.quantity }),
        });
        if (!res.ok) {
